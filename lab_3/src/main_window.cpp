...
MainWindow::MainWindow(QWidget* parent)
{
    ...
    set_n_widget = new TextItem(central_widget, "N", "Enter N");
    set_m_widget = new TextItem(central_widget, "M", "Enter M");
    set_x_widget = new TextItem(central_widget, "X", "Enter X");
    set_y_widget = new TextItem(central_widget, "Y", "Enter Y");
    ...
    regenerate_maze_button->set_text("regenerate maze");
...
void MainWindow::generate_maze(uint8_t n, uint8_t m, uint8_t x, uint8_t y, bool&& is_show)
{
    ...
    for (int i = 0, id = 1, row = 1, column = 1; i < static_cast<int>(n * m) ; ++i, id++)
    {
        ...
        if (swap)
        {
            if (i % 2 == 0)             maze[i]->set_color(91, 62, 39);
            else                        maze[i]->set_color(209, 189, 159);
        }
        else
        {
            if ((i + 1) % 2 == 0)       maze[i]->set_color(91, 62, 39);
            else                        maze[i]->set_color(209, 189, 159);
        }
